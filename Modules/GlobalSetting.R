...
serverGlobalSetting <- function(input, output, session) {
  ...
  rv <- reactiveValues(
    zscore_enabled = TRUE,  # Set default to TRUE (z-score normalization enabled by default)
    settings_changed = FALSE,  # Track if settings have changed
    last_update = Sys.time()  # Timestamp for last update
  )
  
  # Initialize global z-score state on first load
  observe({
      ...
      div(
        ...
        tags$div(
          ...
          tags$ul(
            ...
            tags$li("Drug data: Z-score normalization is applied row-by-row, where each drug is independently normalized across all cell lines"),
            tags$li("Drug-Omic Pair Analysis: A merged dataset plot will be created, combining data from all datasets for a comprehensive view")
          ),
  ...
  observeEvent(input$save_settings, {
    ...
    if (rv$zscore_enabled != input$zscore_checkbox) {
      ...
      # With the new architecture, zscore parameter is passed to load functions
      # so we just need to update the state flag
      assign("GLOBAL_ZSCORE_STATE", list(
      ...
      showNotification(
        HTML(paste(
          "Z-score normalization setting has been", 
          if(rv$zscore_enabled) "enabled" else "disabled",
          "<br><b>Please re-run your analysis to apply the changes.</b>"
        )),
        type = "message",
        duration = 10
      )
